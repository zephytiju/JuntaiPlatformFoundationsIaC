...
try {
  ...
  const required = [
    ...
    "dist/native-full-host-environment.js",
    "dist/native-full-host-network.js",
  ...
  await writeFile(
    ...
    `import foundationsPackage, { compile_reader_exposures, deployNativeVerificationSubstrate, deployNativeFullHostEnvironment, compileNativeFullHostServiceExposures, FOUNDATION_SERVICE_CATALOG, FOUNDATIONS_PACKAGE_VERSION, resolveAndComposeServiceContracts, MeridianRuntimeCapability, MERIDIAN_RUNTIME_DISTRIBUTION, MERIDIAN_DURABLE_RUNTIME_DISTRIBUTION, resolveRuntimeDistribution, validateDomainRequirements, composeDomainRequirements, type DomainRuntimeSelection } from "${packageJson.name}";\n\n` +
      ...
      `if (typeof deployNativeFullHostEnvironment !== "function" || typeof compileNativeFullHostServiceExposures !== "function") throw new Error("missing isolated full-host owner entrypoints");\n` +
