...
The deployment-selected Meridian `structured` binding must support both structured and evidence catalogs, and the `object` binding must support config-artifact object storage. File-backed engine identity, credential, and TLS references must resolve to declared `meridian.runtimeReferences`; those Secret projections are mounted into every consuming workload without putting bytes in Pulumi state. Account, Application Metadata, and Blueprint receive distinct Meridian deployment/config identities so each process discovers only providers shipped in its immutable image. Account's design guarantees are mapped to released adapter primitives: conditional mutation, strong consistency, atomic transactions, transactional evidence append, explicit idempotency receipts, and binding-owned recovery.
