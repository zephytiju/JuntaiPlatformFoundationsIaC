...
| Application Metadata | `3.0.0`   | `/api/platform/applications/v1`, rewritten to internal `/v1`    | Platform supplies Meridian config, workload bindings, Casdoor settings, projected workload and TokenReview tokens, and cluster CA             | Retain the v1 export read-only until cutover acceptance          |
| Blueprint            | `3.0.0`   | `/api/blueprints/v1`                                            | In-process Meridian config-artifact plugin                                                                                                    | Meridian binding owns recovery                                   |
...
The deployment-selected Meridian `structured` binding must support both structured and evidence catalogs. The package registers Account's five co-located structured resources and audit evidence boundary, Application Metadata's four target resources, and Blueprint's resource. Account's design guarantees are mapped to released adapter primitives: conditional mutation, strong consistency, atomic transactions, transactional evidence append, explicit idempotency receipts, and binding-owned recovery.
