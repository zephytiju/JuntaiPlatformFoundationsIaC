...
def main():
    ...
    profile = json.loads(
        Path("runtime/meridian-python-durable/profile.json").read_text()
    )
    assert (
    ...
    assert (output / "requirements.txt").read_bytes() == Path(
        "runtime/meridian-python-durable/requirements.txt"
    ).read_bytes()
    ...
    consumer_lock = Path(
        "runtime/meridian-python-durable/consumer-lock.json"
    ).read_bytes()
