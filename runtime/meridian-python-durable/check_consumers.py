...
Public consumer wheels are ephemeral test inputs and never published in the
...
def main(image, output, cache):
    ...
    with tempfile.TemporaryDirectory(prefix="runtime-consumer-") as temporary:
        ...
        shutil.copyfile(source / "check_bindings.py", root / "check_bindings.py")
...
""")
        ...
        try:
            ...
            print(
                f"Accepted {len(result['importedModules'])} installed public modules; evidence: {output}"
            )
        finally:
            subprocess.run(
